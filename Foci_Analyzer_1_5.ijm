...
#@ String (label = "3D image handling", choices={"Analyze foci in 3D", "Detect foci on a Extended Depth of Focus Projection", "Detect foci on the Maximum Intensity Projection", "Use quasi-2D foci detection (detect foci separately in every Z-slice)", "Process a single z-slice only (specify below which slice)"}, style="radioButtonVertical") ThreeDHandling
...
#@ Integer (label = "Image XY binning before analysis [1-n]", value = 1, min=1, description="Image binning will greatly shorten the analysis time, but can be less accurate.") XYBinning

#@ String (value="Nuclei/cell detection settings", visibility="MESSAGE") nuclei_message
#@ String (label = "Nuclei/cell segmentation method", choices={"StarDist nuclei segmentation (recommended)","Cellpose cytoplasm segmentation", "Classic nuclei segmentation", "Load ROIs from file"}, style="listBox") nucleiSegmentationChoice
#@ Double (label = "Stardist nuclei rescaling factor [1-n], 0 for automatic rescaling", value = 0, min=0, description="A binning >1 for high-resolution input images can prevent 'oversegmentation', and will also somewhat speed up processing.") downsampleFactorStarDist
#@ Double (label = "Probablility threshold [0.0-1.0] (StarDist/Cellpose)", value = 0.5, min=0, max=1, style="format:0.0", description="lower values will accept more nuclei") probabilityThreshold
#@ Integer (label = "Cellpose cell diameter (pixels), 0 for automatic", value = 0, min=0, description="Estimation of the cell diameter") cellPoseDiameter
...
#@ Integer (label = "Minimum foci size (pixels/voxels)", value = 3) minFociSize
#@ Integer (label = "Maximum foci size (pixels/voxels)", value = 9999) maxFociSize

#@ Double (label = "Include foci outside nuclei/cells, with maximum distance (units); -1 for the full image", value = 0, min=-1, style="format:0.0") maxFociDistanceOutsideNuclei_setting
 ...
 * 
 * Load settings from a previously analyzed image
 * 
 */
...
if (nucleiSegmentationChoice == "Load ROIs from file") {
	...
	Dialog.addDirectory("Folder containing ROI .zip files", File.getParent(files[0]));
	Dialog.show();
...
//TO DO: FIND A BETTER WAY FOR THIS:
	...
	if(downsampleFactorStarDist == 0 && unit == "µm" || unit == "um" || unit == "microns" || unit == "micron") {
		//if(0.5/pixelWidth > 0.5) downsampleFactorStarDist = maxOf(floor(0.5/pixelWidth), 1);
		//else downsampleFactorStarDist = 0.5/pixelWidth;
		downsampleFactorStarDist = 0.5/pixelWidth;
	...
	else if (downsampleFactorStarDist == 0) print("WARNING: Pixel unit is not µm. Nuclei will not be downscaled for StarDist.");
	else {
		if(downsampleFactorStarDist < 0.3333) {
			print("WARNING: Overruling StarDist downsample factor ("+downsampleFactorStarDist+"). Will be set to the minimum of 0.33");
			downsampleFactorStarDist = 0.3333;	//Allow upscaling up to 3x
		}
	...
	if(nrNuclei > 0) {
		...
		if (nucleiSegmentationChoice == "Load ROIs from file") {
			for (i = 0; i < nrNuclei; i++) {
				roiManager("select", i);
				Table.set("Nucleus UUID", i, Roi.getName);
			}
			roiManager("deselect");
		}
		...
		if(maxFociDistanceOutsideNuclei_setting != 0) {
			...
			//N.B. For 3D data this dilation step is not entirely fair, because labelmap _nuclei-3D is actually an extended 2D labelmap, which will then be dilated in 3D, with non-isotropic pixels. Oh well..
		}
		...
		if(maxFociDistanceOutsideNuclei_setting != 0) {
			...
			dilated_nuclei_outlines = "dilated_nuclei_outlines";
		...
		do {
			...
			//Foci channel A
			detections_fociA = detect_foci(process_image, fociChannelA, fociSizeA, anisotropyFactor, firstTimeProcessing, labelmap_nuclei, labelmap_nuclei_3D, thresholdFactorA);
			...
			if(detectFociChannelB == true) {
				detections_fociB = detect_foci(process_image, fociChannelB, fociSizeB, anisotropyFactor, firstTimeProcessing, labelmap_nuclei, labelmap_nuclei_3D, thresholdFactorB);
...
//		Stack.setChannel(2);
	...
	//			Ext.CLIJ2_reportMemory();
			...
			List.set("22. Actual threshold value channel A ", thresholdA);
			List.set("23. Actual threshold value channel B ", thresholdB);
			List.set("24. Minimum foci size ", minFociSize);
			List.set("25. Maximum foci size ", maxFociSize);
			List.set("26. Include foci outside nuclei, with maximum distance (units) ", maxFociDistanceOutsideNuclei_setting);
			List.set("27. Minimum overlap of foci to colocalize (pixels/voxels) ", minOverlapSize);
			list = List.getList();
			...
			if(saveOverlayImage == true) {
				if(detectFociChannelB == true) {
					...
					saveAs("zip", outputFolder + File.separator + imageName + "__foci_overlap_map");
				}
			}
...
function loadROIs(image, channel, ROIsFolder) {
//	File.setDefaultDir(outputFolder);
	run("Duplicate...", "title=nuclei duplicate channels=" +channel);	//Get nucleus channel

	roiManager("reset");
	roiManager("open", ROIsFolder+File.separator+File.getNameWithoutExtension(image)+"_ROIs.zip");
	run("ROI Manager to LabelMap(2D)");
	labelmap_nuclei_final = "Labelmap_nuclei";
...
function segmentCytoplasmCellpose (image, channel, probabilityThreshold, pixelWidth, unit, resultTable) {
	...
	run("Cellpose Advanced", "diameter="+cellPoseDiameter+" cellproba_threshold=0 flow_threshold="+probabilityThreshold+" anisotropy=1.0 diam_threshold=12.0 model=cyto2 nuclei_channel=0 cyto_channel=1 dimensionmode=2D stitch_threshold=-1.0 omni=false cluster=false additional_flags=");
...
function detect_foci(image, channel, fociSize, anisotropyFactor, firstTimeProcessing, labelmap_nuclei, labelmap_nuclei_3D, thresholdFactor) {
...
//Ext.CLIJx_detectAndLabelMaximaAboveThreshold(foci, detectedMaxima, 0, 0, 0, threshold, false)
	...
	Ext.CLIJ2_mask(labelmapDetectedFociFiltered, labelmap_nuclei_3D_dilated, labelmapDetectedFociFilteredInsideNuclei);
...
//			print("dimensions of nucleus "+i+1+" image: "+width+ ", "+height);
		...
		for(k = 0; k < nrFoci; k++) {
			...
			size_[k] = getResult("PIXEL_COUNT", nrResults + k) * pixelWidth * pixelHeight * pixelDepth;
			sumIntensity = sumIntensity + getResult("SUM_INTENSITY", nrResults + k);
			sumSize = sumSize + size_[k];
		}
		...
		foci_sum_int_[i] = sumIntensity;
		foci_sum_size_[i] = sumSize;
...
function create_overlap_map(overlap_percentage_, labelmap_nuclei_3D, overlapMapName) {
	...
	Ext.CLIJ2_release(overlap_fill);
		if(debugMode == true) showImagefromGPU(overlap_map);
